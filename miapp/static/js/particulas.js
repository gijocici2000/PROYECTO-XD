...
var ParticleEngine = (function() {
    ...
    for (var i = 0, len = _ParticleEngine.particleSettings.length; i < len; i++) {
        ...
        for (var s = 0; s < ball.num; s++ )
        {
            ...
            if(ball.fill){
                // Colores vivos para las partículas
                var colores = ["#ff3b3b", "#00e6e6", "#ffe600", "#ff00cc", "#00ff00", "#0099ff", ball.color];
                var colorElegido = colores[Math.floor(Math.random() * colores.length)];
                circle.graphics.beginFill(colorElegido).drawCircle(0, 0, ball.ballwidth);
		...
		// Fondo gris oscuro para el canvas
        var bg = new createjs.Shape();
        bg.graphics.beginFill("#23272b").drawRect(0, 0, this.totalWidth, this.totalHeight);
    ...
    // Ajustar el tamaño real del canvas a la ventana
    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight;
    this.totalWidth = this.canvasWidth = canvas.width;
    this.totalHeight = this.canvasHeight = canvas.height;

    // Ajustar el fondo al nuevo tamaño
    if (this.stage.children[0]) {
        this.stage.children[0].graphics
            .clear()
            .beginFill("#191d1e")
            .drawRect(0, 0, this.totalWidth, this.totalHeight);
    }
